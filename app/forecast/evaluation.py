...
Metrics over withheld hours that have an observation: MAE (kWh per hour)
and aggregate energy error (sum predicted − sum actual, kWh, plus its
absolute value). No percentage metrics (zero denominators are possible).
"""
...
class Score:
    ...
    aggregate_error_kwh: float  # predicted total − actual total over scored hours

    @property
    def abs_aggregate_error_kwh(self) -> float:
        return abs(self.aggregate_error_kwh)
...
def _score(method: str, predicted: dict[datetime, float], actual: dict[datetime, float]) -> Score:
    common = [t for t in actual if t in predicted]
    errors = [predicted[t] - actual[t] for t in common]
    return Score(method, len(common), sum(abs(e) for e in errors) / len(common) if common else float("nan"), sum(errors))
...
def holdout(history: dict[datetime, float], cutoff: datetime, horizon: str, tz: ZoneInfo, working_days: set[int]) -> dict[str, Score]:
    ...
    return {"baseline": _score("baseline", baseline, both), "repeat_last_day": _score("repeat_last_day", naive, both)}
